...
\label{*}
\end{equation}
The proof goes in two steps: we first construct a variable $X''$ form $X'$ that approximate $X$ as $X'$ does, i.e., $X \preceq_{\varepsilon^*,m} X''$, but also has the property that its support is a subset of the support of $X$. Then, form $X''$, we construct another random variable, $X'''$, that on top of being an approximation of $X$ with the same parameters is also equal to $X_P$ for some partition $P$. 
...
Since we only assign a non-zero probability to $Pr(X''=t)$ if $t=t_0$ or if $t-t_i$, i.e., only if $t$ is in the support of $X$, we have that $\support(X'') \subseteq \support(X)$ by construction. Furthermore, by construction of $X''$, $|\support(X'')|\leq m$ since $|\support(X')|\leq m$ and every $ t\in \support(X')$ is mapped to a 
$ Gera: I don't understand the following sentence :-(
