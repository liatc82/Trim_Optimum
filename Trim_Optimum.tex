...
\section{Introduction}
 \begin{itemize}
 	...
 	\item More specifically, we say that $X'$ is an $\varepsilon$-approximation of $X$ if $|X-X'|_{K} \leq \varepsilon$ and $F_X < F_{X'}$. The second part is because we want a safe estimation of the probability of missing a deadline, i.e., we only allow ``false positive'' errors where our estimation is above the real value, not below it.
