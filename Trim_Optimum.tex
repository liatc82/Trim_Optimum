...
Second, we prove that $F_{X'''}(t) \leq F_{X''}(t)$ by showing $F_{X''}(t) - F_{X'''}(t)\geq 0$. Lets look at the maximal $s' \in (s_i,s_{i+1})$. From Claim~\ref{suppGMin} $F_{X''}(s_i)=F_{X''}(s')$ .

\begin{align*}
&F_{X''}(s')-F_{X}(s')\geq 0 \Longleftrightarrow\\
&F_{X''}(s) - F_{X}(s)-Pr(s_i<X< s_{i+1})\geq 0 \Longleftrightarrow\\
&F_{X''}(s) - F_{X'''}(s)\geq 0 \\
\end{align*}
