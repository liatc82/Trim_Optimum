...
Second, we prove that $F_{X'''}(t) \leq F_{X''}(t)$ by showing $F_{X''}(t) - F_{X'''}(t)\geq 0$ 

\begin{align*}
&F_{X''}(s_i) - F_{X'''}(s_i)\geq 0\\
&F_{X}(s_i)+\varepsilon' - F_{X}(s_i)+Pr(s_i<X< s_{i+1})\geq 0\\
&\varepsilon'-Pr(s_i<X< s_{i+1})\geq 0
\end{align*}
